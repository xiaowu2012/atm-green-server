...
//¾ÐÃà 
//CCompressManager jcm;  // ·Î±×ÀÎ ÇÒ°æ¿ì µ¥ÀÌÅÍ ¾ÐÃà & ÇØÁ¦ 
// ¾ÏÈ£È­ °ü·Ã
...
//	if(m_nDBPort == 0)
//	{
//		::MessageBox(NULL, "ERROR: LOGIN_SERVER PORT", "º¯¼ö ¿¡·¯", MB_OK);
//		return FALSE;
//	}
//
//	// DB-ADDRESS ¾ò±â...
...
//	{
//		::MessageBox( NULL, "ERROR: LOGIN_SERVER_ADDRESS", "º¯¼ö ¿¡·¯", MB_OK );
//		return FALSE;
//	}
//
//	m_pDBSocket = NULL;
...
//	{
//		bIsZoneClient = TRUE;	// Zone Client ·Î µ¿ÀÛ
//	}
//	//
	...
	// Socket for Server IOCP Base ÃÊ±âÈ­...
	...
	// Pool for UserBase ÃÊ±âÈ­...   wait ¿ªÈ° 
...
/*
	...
	if(!DBSocketConnect())
	{
		::MessageBox(NULL, "Login Server ¸¦ ¸ÕÀú ½ÇÇàÇØ ÁÖ½Ê½Ã¿ä.", "Error", MB_OK);
		return FALSE;
	}
...
UINT WorkerLoginThread( void *lp )
{
	...
	for (;;)
	{
		...
		{
			Sleep(1);//2015ÄêMarkJay ÐÞ¸Ä Ä¬ÈÏ1
			continue;
		}
			...
			if ( pUser && pUser->m_SockFlag == 1 )
			{
				switch(pLIOTD->CODE)
				{
					case ACCOUNT_LOGIN_REQ:
						//pUser->AccountLoginReq(pLIOTD->ID);//²âÊÔÐÞ¸´µô¶Ë¿ªÊ¼
						break;
...
UINT WorkerLogoutThread( void *lp )
{
	...
	for (;;)
	{
		...
		while( nLocalDataCount >= 1 )
		{
			...
			if(pUser) pUser->SockCloseProcess();
			delete pLIOTD;
			//Sleep(1);//MarkJay ÐÞ¸Ä ±¾ÉíÒÑ×¢Ïú
		}
...
UINT WorkerFortressThread( void *lp )
{
	...
	for (;;)
	{
		...
		while( nLocalDataCount >= 1 )
		{
			...
			if(pFort->m_lUsed != 1)			// °ø¼ºÀüÀÌ ¾Æ´Ï¸é..Àß¸øµÈ Á¤º¸ÀÓ
			{
				delete pSDP;
				Sleep(1);//MarkJay ÐÞ¸Ä Ä¬ÈÏ1
				continue;
			...
			strMsg.Format( IDS_USER_GET_FORTRESS_SUCCESS, pSDP->FORTRESS );//"%s ¾üÍÅ»ñµÃÒªÈû."
			...
			for(i = 0; i < MAX_USER; i++)
			{
				...
				if (  pUser->m_SockFlag == 1 )
				{
					...
					{												//±æµåÀüÀÌ ÀÏ¾î³ª´Â »óÁ¡Áö¿ªÀÌ¸é
						...
						if(g_arMapTable[mapindex] == NULL) continue;//MarkJay ÐÞ¸Ä
...
void CServerDlg::InitThreads()//³õÊ¹»¯Ïß³Ì
{
	int i = 0;
	...
	m_hLoginThread = AfxBeginThread( WorkerLoginThread, (LPVOID)this );//µÇÂ½Ïß³Ì¿ªÊ¼

	m_hLogoutThread = AfxBeginThread( WorkerLogoutThread, (LPVOID)this );//ÍË³öÏß³Ì¿ªÊ¼

	m_hSqlThread = AfxBeginThread( WorkerSQLThread, (LPVOID)this );//SQLÏß³Ì¿ªÊ¼

	m_hFortressThread = AfxBeginThread( WorkerFortressThread, (LPVOID)this );//±¤ÀÝÏß³Ì¿ªÊ¼

	// Ã¤ÆÃ ½º·¹µå »ý¼º...
	for ( i = 0; i < AUTOMATA_THREAD; i++ )
	{
		m_ctThread[i] = AfxBeginThread( WorkerChatThread, (LPVOID)&m_ThreadNo[i] );//Ëµ»°Ïß³Ì¿ªÊ¼
	}
}
...
void CServerDlg::SetAliveSignalFill()
{
	...
	for ( int i = 0; i < MAX_USER; i++ )
	{
		...
		if ( pUser && pUser->m_state != STATE_GAMESTARTED )
		{
			...
			if ( curtick - pUser->m_ConnectionSuccessTick > 3000)//¿¨ºÅ½â¾ö30ÃëºóÉÏ
				pUser->SoftClose();
		}
	}
}
...
void CServerDlg::OnBnClickedQuanout()
{
...
/*void CServerDlg::OnBnClickedButtonDrop() //ÖØÔØµô±¦ Ð¡ºÚ
{
     GetNpcItemNewTable();
