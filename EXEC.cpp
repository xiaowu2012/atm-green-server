...
void EXEC::Parse(char *pBuf)
{
	...
	else if( !strcmp( temp, "CHOUJIANG" ) ) // NpcÔª±¦ÉÌµê
	{
		m_Exec = EXEC_CHOUJIANG;
	}
	else if( !strcmp( temp, "NPC_SHUXING" ) )
	{
		index += ParseSpace( temp, pBuf+index );	m_ExecInt[i++] = atoi( temp );
		index += ParseSpace( temp, pBuf+index );	m_ExecInt[i++] = atoi( temp );
		m_Exec = EXEC_NPC_SHUXING;//¶Ä²©Âò´ó
	}
	else if( !strcmp( temp, "NPC_DA" ) )
	{
		index += ParseSpace( temp, pBuf+index );	m_ExecInt[i++] = atoi( temp );
		index += ParseSpace( temp, pBuf+index );	m_ExecInt[i++] = atoi( temp );
		m_Exec = EXEC_NPC_DA;//¶Ä²©Âò´ó
	}
	else if( !strcmp( temp, "NPC_XIAO" ) )
	{
		index += ParseSpace( temp, pBuf+index );	m_ExecInt[i++] = atoi( temp );
		index += ParseSpace( temp, pBuf+index );	m_ExecInt[i++] = atoi( temp );
		m_Exec = EXEC_NPC_XIAO;//¶Ä²©ÂòÐ¡
	}
	else if( !strcmp( temp, "NPC_ZHUANZHI" ) )//×ªÖ°NPC¶Ô»°
	{
		index += ParseSpace( temp, pBuf+index );	m_ExecInt[i++] = atoi( temp );
		m_Exec = EXEC_NPC_ZHUANZHI;
	}
	else if( !strcmp( temp, "NPC_SHANGDIAN" ) )//×ª»»ÉÌµêÄ£Ê½NPC
	{
		index += ParseSpace( temp, pBuf+index );	m_ExecInt[i++] = atoi( temp );
		m_Exec = EXEC_NPC_SHANGDIAN;
	}
	else if( !strcmp( temp, "MARKHUODONG" ) )//ËÀÍöÁìµØNPC¶Ô»°
	{
		index += ParseSpace( temp, pBuf+index );	m_ExecInt[i++] = atoi( temp );
		m_Exec = EXEC_MARKHUODONG;
	}
	else if( !strcmp( temp, "MARK_CZ_SAY" ) )//ÔÚÏß³äÖµ¶Ô»°¿ò
	{
		m_Exec = EXEC_MARK_CZ_SAY;
	}
	else if( !strcmp( temp, "NEWGONGNENG" ) )//ÐÂ½Å±¾µ÷ÓÃ MarkJay
	{
		index += ParseSpace( temp, pBuf+index );	m_ExecInt[i++] = atoi( temp );
		m_Exec = EXEC_NEWGONGNENG;
	}
	else if(!strcmp(temp,"DUIHUANITEM"))//¶Ò»»ÓÐÊôÐÔÎïÆ·
	{
		m_Exec = EXEC_DUIHUANITEM;
		index += ParseSpace( temp, pBuf+index );	m_ExecInt[i++] = atoi( temp ); 
		index += ParseSpace( temp, pBuf+index );	m_ExecInt[i++] = atoi( temp ); 
		index += ParseSpace( temp, pBuf+index );	m_ExecInt[i++] = atoi( temp ); 
		index += ParseSpace( temp, pBuf+index );	m_ExecInt[i++] = atoi( temp ); 
	...
	else if( !strcmp( temp, "ADD4PAI" ) )//3ÅÅºÏ³É4ÅÅ MarkJay
	{
		index += ParseSpace( temp, pBuf+index );	m_ExecInt[i++] = atoi( temp );
		m_Exec = EXEC_ADD4PAI;
	}
	else if( !strcmp( temp, "ADD4PWQ" ) )//3ÅÅºÏ³É4ÅÅ MarkJay
	{
		index += ParseSpace( temp, pBuf+index );	m_ExecInt[i++] = atoi( temp );
		m_Exec = EXEC_ADD4PWQ;
	}

	else if( !strcmp( temp, "ADD4SS" ) )//¶Ä²©Ê×ÊÎ
	{
		index += ParseSpace( temp, pBuf+index );	m_ExecInt[i++] = atoi( temp );
		m_Exec = EXEC_ADD4SS;
	}
	else if( !strcmp( temp, "ON_WEB" ) ) //ÔÚÏßÍøÒ³
	{
		m_Exec = EXEC_ON_WEB;		
	}	
	...
	else if( !strcmp( temp, "MONEYSHOP" ) )//´ò¿ªÔª±¦ÉÌµê  ÏÔÊ¾ÊôÐÔ  ½Å±¾
	{
		m_Exec = EXEC_MONEYSHOP;

		index += ParseSpace( temp, pBuf+index );	m_ExecInt[i++] = atoi( temp );
		index += ParseSpace( temp, pBuf+index );	m_ExecInt[i++] = atoi( temp );
	}
	else if( !strcmp( temp, "EMO" ) )
	{
		m_Exec = EXEC_EMO;
	}
	else if( !strcmp( temp, "ZHANCHANG" ) )
	{
		m_Exec = EXEC_ZHANCHANG;
	}
	...
	else if( !strcmp( temp, "ROB_YUANBAO" ) )//NPCÊÕÔª±¦
	{
		m_Exec = EXEC_ROB_YUANBAO;
	...
	//else if( !strcmp( temp, "GIVE_YUANBAO" ) )//NPC¸øÔª±¦
	//{
	//	m_Exec = EXEC_GIVE_YUANBAO;
	...
	else if( !strcmp( temp, "MARK_ROBITEM" ) )//Ð¡ºÚÐÞÁ¶
	{
		m_Exec = EXEC_MARK_ROBITEM;
		index += ParseSpace( temp, pBuf+index );	m_ExecInt[i++] = atoi( temp );
	}
	else if( !strcmp( temp, "MARK_CZ" ) )
	{
		m_Exec = EXEC_MARK_CZ;
	}
}
