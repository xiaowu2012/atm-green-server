...
BOOL				g_isBaoZangKG	= FALSE;//½ûÖ¹PK¶´¿ª¹Ø

BOOL				GuildWarEnd		= FALSE;//ÒªÈû½áÊø MarkJay
...
OnlineShopArray				g_arOnlineShopTable;//ÔÚÏßÉÌµê
VIPPayArray					g_arVIPPayTable;//»áÔ±³äÖµ
MarkPayArray				g_arMarkPayTable;//ÔÚÏß³äÖµ
MarkGonggaoArray			g_arMarkGonggaoTable;//ÔÚÏß¹«¸æ
MarkChouJiangArray			g_arMarkChouJiangTable;//ÔÚÏß³é½±
...
CFile g_fpSpeedHack;
CFile g_fpShuaWuPin;//Ë¢ÎïÆ·¼ÇÂ¼
CFile g_fpMarkDeBug;//MarkJayÂ©¶´ÐÅÏ¢
CFile g_fpMarkShiQuLog;//Ê°È¡ÈÕÖ¾
...
BEGIN_MESSAGE_MAP(CServerDlg, CDialog)
	...
	ON_BN_CLICKED(IDQUANOUT, &CServerDlg::OnBnClickedQuanout)
	ON_BN_CLICKED(IDC_NEW_ANNOUNCE,&CServerDlg::OnBnClickedButtonDrop)//ÖØÔØµô±¦ Ð¡ºÚ
END_MESSAGE_MAP()
...
//		g_DBSession[i].Init(2);
	...
	// Speed Hack File
	g_fpSpeedHack.Open( "./·þÎñ¶ËÈÕÖ¾/SpeedHackList.txt", CFile::modeWrite | CFile::modeCreate | CFile::modeNoTruncate | CFile::shareDenyNone );
	g_fpSpeedHack.SeekToEnd();

	// Speed Hack File
	g_fpShuaWuPin.Open( "./·þÎñ¶ËÈÕÖ¾/GM¼ÇÂ¼.txt", CFile::modeWrite | CFile::modeCreate | CFile::modeNoTruncate | CFile::shareDenyNone );
	g_fpShuaWuPin.SeekToEnd();

	g_fpMarkDeBug.Open( "./·þÎñ¶ËÈÕÖ¾/DeBugµ÷ÊÔÐÅÏ¢.txt", CFile::modeWrite | CFile::modeCreate | CFile::modeNoTruncate | CFile::shareDenyNone );
	g_fpMarkDeBug.SeekToEnd();

	g_fpMarkShiQuLog.Open( "./·þÎñ¶ËÈÕÖ¾/ÎïÆ·Ê°È¡ÈÕÖ¾.txt", CFile::modeWrite | CFile::modeCreate | CFile::modeNoTruncate | CFile::shareDenyNone );
	g_fpMarkShiQuLog.SeekToEnd();

	g_fpBackServer.Open( "./·þÎñ¶ËÈÕÖ¾/BackServerLog.txt", CFile::modeWrite | CFile::modeCreate | CFile::modeNoTruncate | CFile::shareDenyNone );
	g_fpBackServer.SeekToEnd();

	g_fpGuildHouseRank.Open( "./·þÎñ¶ËÈÕÖ¾/GuildHouseRankLog.txt", CFile::modeWrite | CFile::modeCreate | CFile::modeNoTruncate | CFile::shareDenyNone );
	g_fpGuildHouseRank.SeekToEnd();

	g_fpEventItem.Open( "./·þÎñ¶ËÈÕÖ¾/EventItemLog.txt", CFile::modeWrite | CFile::modeCreate | CFile::modeNoTruncate | CFile::shareDenyNone );
	...
	if(!GetServerInfoData())
	{
  	...
  	//----------------------------------------------------------------------
	if(!GetLoadMarkJzINI())//Markini//iniÎÄ¼þ
	{
		EndDialog(IDCANCEL);
		return FALSE;
	}
	//----------------------------------------------------------------------
	if(!GetLoadMarkGMINI())//MarkGMini//iniÎÄ¼þ
	{
		EndDialog(IDCANCEL);
		return FALSE;
	}
...
//	AfxMessageBox("8");
	...
	if( !GetOnlineShopTable())//ÔÚÏßÉÌµê
	{
		EndDialog(IDCANCEL);
		return FALSE;
	}
	if( !GetVIPPayTable())//»áÔ±³äÖµ
	{
		EndDialog(IDCANCEL);
		return FALSE;
	}
	if( !GetMarkPayTable())//ÔÚÏß³äÖµ
	{
		EndDialog(IDCANCEL);
		return FALSE;
	}
	if( !GetMarkGonggaoTable())//ÔÚÏß¹«¸æ
	{
		EndDialog(IDCANCEL);
		return FALSE;
	}
	if( !GetMarkChouJiangTable())//ÔÚÏß³é½±
	{
		EndDialog(IDCANCEL);
		return FALSE;
	}
...
//	TRACE("ÃÑ %d NPC Áß ÇöÀç %d NPC ÃÊ±âÈ­°¡ ³¡³µ½À´Ï´Ù.\n", g_TotalNPC, g_CurrentNPC);
	...
	{
		AfxMessageBox( "²»ÄÜ´ò¿ªÇÅ·þÎñÆ÷" );//MarkJay ÐÞ¸Ä
		return FALSE;
	}
	...
	// »ç¿ëÀÚ Á¢¼Ó ÃÊ±âÈ­...
	if ( !m_UserBase.Listen( g_server[nServerIndex]->m_nPort ) )  //¶Ë¿Ú¼àÌý MarkJay
		return FALSE;

	// Å¸ÀÌ¸Ó ¼¼ÆÃ...
	m_nAliveTimerID = SetTimer( ALIVE_FILL_TIMER, 3000, NULL );

//	PostMessage(UM_START_SERVER,0,0);

	return TRUE;  // return TRUE  unless you set the focus to a control
}
BOOL CServerDlg::GetLoadMarkJzINI()//iniÎÄ¼þ
{
	TCHAR strPath[_MAX_PATH + 1];
	::GetCurrentDirectory(_MAX_PATH, strPath);
	CString szfilename = strPath;
	szfilename += _T("\\ÓÎÏ·ÉèÖÃ.ini");	
	CString tttmp;
		
	MarkIni *fcini = new MarkIni;
	GetPrivateProfileString("·þÎñÆ÷", "×î¸ßµÈ¼¶",  "", tttmp.GetBuffer(MAX_PATH),24, szfilename);     fcini->TJ_JGTJ = atoi(tttmp);
	GetPrivateProfileString("·þÎñÆ÷", "×î¸ß¼ÓµãÖµ",  "", tttmp.GetBuffer(MAX_PATH),24, szfilename);	  fcini->JG_DX = atoi(tttmp);
	GetPrivateProfileString("·þÎñÆ÷", "¾­Ñé±¶Êý",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->ioExp = atoi(tttmp);
	GetPrivateProfileString("·þÎñÆ÷", "Õ½¶·¾­Ñé",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->idwXP = atoi(tttmp);	
	GetPrivateProfileString("·þÎñÆ÷", "ÓÎÏ·Ãû×Ö",  "", tttmp.GetBuffer(MAX_PATH),MAX_PATH, szfilename);     strcpy(fcini->GameName,tttmp);
	GetPrivateProfileString("·þÎñÆ÷", "130¼¼ÄÜ¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->KG130 = atoi(tttmp);
	GetPrivateProfileString("·þÎñÆ÷","³äÖµÑ¡Ïî","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);  fcini->CZ_YZB_YB = atoi(tttmp);
	GetPrivateProfileString("·þÎñÆ÷","ÉÌ³ÇÖØµþ¿ª¹Ø","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);  fcini->ShopKG = atoi(tttmp);
	GetPrivateProfileString("·þÎñÆ÷", "Ð¡ºÅ±£»¤Ïà²îµÈ¼¶",  "", tttmp.GetBuffer(MAX_PATH),24, szfilename); fcini->XHBH_DJ = atoi(tttmp);
	GetPrivateProfileString("·þÎñÆ÷", "Èº¹¥¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->QunGong_KG = atoi(tttmp);
	GetPrivateProfileString("·þÎñÆ÷", "ÆÀ·Ö¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->PingFen_KG = atoi(tttmp);
	GetPrivateProfileString("·þÎñÆ÷", "Èý±¶ÈËÊý¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->RenShu_KG = atoi(tttmp);
	GetPrivateProfileString("·þÎñÆ÷", "Ë¢ÎïÆ·ÊýÁ¿ÉèÖÃ",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->ShuaWuPin_Num = atoi(tttmp);
	GetPrivateProfileString("·þÎñÆ÷", "ÉÌµêÄ£Ê½ÇÐ»»¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->Shop_Qh_KG = atoi(tttmp);
	GetPrivateProfileString("·þÎñÆ÷", "ÊÇ·ñÏÞÖÆ·è¿ñ¼ÓÑª",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->Wg_HP_KG = atoi(tttmp);
    GetPrivateProfileString("·þÎñÆ÷", "ÎïÀí±ØÉ±ÉËº¦±¶ÂÊ",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->Wl_EBODYSkill = atoi(tttmp);
	GetPrivateProfileString("·þÎñÆ÷", "·¨Ê¦±ØÉ±ÉËº¦±¶ÂÊ",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->Fs_EBODYSkill = atoi(tttmp);
	GetPrivateProfileString("·þÎñÆ÷", "ÔÚÏßÍøÒ³",  "", tttmp.GetBuffer(MAX_PATH),MAX_PATH, szfilename);     strcpy(fcini->WEBURL,tttmp);

	GetPrivateProfileString("¶Ä²©","ÎäÆ÷3±ä4Ç®Êý","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->WQBZSL = atoi(tttmp);
	GetPrivateProfileString("¶Ä²©","ÎäÆ÷4±ä3Ç®Êý","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->JJWQSL = atoi(tttmp);
	GetPrivateProfileString("¶Ä²©","ÎäÆ÷³É¹¦ÂÊ","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->WQCG = atoi(tttmp);
	GetPrivateProfileString("¶Ä²©","ÎäÆ÷Ê§°ÜÂÊ","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->WQSB = atoi(tttmp);
	GetPrivateProfileString("¶Ä²©","×°±¸3±ä4Ç®Êý","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->ZHBZSL = atoi(tttmp);
	GetPrivateProfileString("¶Ä²©","×°±¸4±ä3Ç®Êý","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->JJBZSL = atoi(tttmp);
	GetPrivateProfileString("¶Ä²©","Ê×ÊÎ3±ä4Ç®Êý","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->SSSJBZSL = atoi(tttmp);
	GetPrivateProfileString("¶Ä²©","Ê×ÊÎ4±ä3Ç®Êý","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->SSJJBZSL = atoi(tttmp);
	GetPrivateProfileString("¶Ä²©","×°±¸³É¹¦ÂÊ","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->ZBCG = atoi(tttmp);
	GetPrivateProfileString("¶Ä²©","×°±¸Ê§°ÜÂÊ","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->ZBSB = atoi(tttmp);
	GetPrivateProfileString("¶Ä²©","Ê×ÊÎ³É¹¦ÂÊ","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->SSCG = atoi(tttmp);
	GetPrivateProfileString("¶Ä²©","Ê×ÊÎÊ§°ÜÂÊ","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->SSSB = atoi(tttmp);

	GetPrivateProfileString("¹ÖÎïÊýÁ¿ÏÞÖÆ", "150-155ÊýÁ¿",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->GuaiWuNum_150_155 = atoi(tttmp);
	GetPrivateProfileString("¹ÖÎïÊýÁ¿ÏÞÖÆ", "155-160ÊýÁ¿",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->GuaiWuNum_155_160 = atoi(tttmp);

	GetPrivateProfileString("Ì××°ºÐ×Ó", "ºÐ×Ó¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->ITEM_GM_KG = atoi(tttmp);

	GetPrivateProfileString("¾­Ñé±¶Êý","¾­Ñé¿ØÖÆ¿ª¹Ø","",tttmp.GetBuffer(MAX_PATH), 24,szfilename); fcini->Exp_Kz_Kg = atoi(tttmp);
	GetPrivateProfileString("¾­Ñé±¶Êý","iNpcExp_50","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iNpcExp_50 = atoi(tttmp);	//50¼¶ÒÔÏÂµÄ¾­Ñé±¶Êý32
	GetPrivateProfileString("¾­Ñé±¶Êý","iNpcExp_60","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iNpcExp_60 = atoi(tttmp);	//60¼¶ÒÔÏÂµÄ¾­Ñé±¶Êý16
	GetPrivateProfileString("¾­Ñé±¶Êý","iNpcExp_70","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iNpcExp_70 = atoi(tttmp);	//70¼¶ÒÔÏÂµÄ¾­Ñé±¶Êý8
	GetPrivateProfileString("¾­Ñé±¶Êý","iNpcExp_80","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iNpcExp_80 = atoi(tttmp);	//80¼¶ÒÔÏÂµÄ¾­Ñé±¶Êý4
	GetPrivateProfileString("¾­Ñé±¶Êý","iNpcExp_90","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iNpcExp_90 = atoi(tttmp);	//90¼¶ÒÔÏÂµÄ¾­Ñé±¶Êý2
	GetPrivateProfileString("¾­Ñé±¶Êý","iNpcExp_99","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iNpcExp_99 = atoi(tttmp);	//99¼¶ÒÔÏÂµÄ¾­Ñé±¶Êý2
	GetPrivateProfileString("¾­Ñé±¶Êý","iNpcExp_100","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iNpcExp_100 = atoi(tttmp);	//100¼¶ÒÔÏÂµÄ¾­Ñé±¶Êý
	GetPrivateProfileString("¾­Ñé±¶Êý","iNpcExp_104","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iNpcExp_104 = atoi(tttmp);	//104¼¶ÒÔÏÂµÄ¾­Ñé±¶Êý
	GetPrivateProfileString("¾­Ñé±¶Êý","iNpcExp_105","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iNpcExp_105 = atoi(tttmp);	//105¼¶ÒÔÏÂµÄ¾­Ñé±¶Êý
	GetPrivateProfileString("¾­Ñé±¶Êý","iNpcExp_106","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iNpcExp_106 = atoi(tttmp);	//106¼¶ÒÔÏÂµÄ¾­Ñé±¶Êý
	GetPrivateProfileString("¾­Ñé±¶Êý","iNpcExp_107","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iNpcExp_107 = atoi(tttmp);	//107¼¶ÒÔÏÂµÄ¾­Ñé±¶Êý
	GetPrivateProfileString("¾­Ñé±¶Êý","iNpcExp_108","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iNpcExp_108 = atoi(tttmp);	//108¼¶ÒÔÏÂµÄ¾­Ñé±¶Êý
	GetPrivateProfileString("¾­Ñé±¶Êý","iNpcExp_109","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iNpcExp_109 = atoi(tttmp);	//109¼¶ÒÔÏÂµÄ¾­Ñé±¶Êý
	GetPrivateProfileString("¾­Ñé±¶Êý","iNpcExp_110","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iNpcExp_110 = atoi(tttmp);	//110¼¶ÒÔÏÂµÄ¾­Ñé±¶Êý
	GetPrivateProfileString("¾­Ñé±¶Êý","iNpcExp_115","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iNpcExp_115 = atoi(tttmp);	//115¼¶ÒÔÏÂµÄ¾­Ñé±¶Êý
	GetPrivateProfileString("¾­Ñé±¶Êý","iNpcExp_120","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iNpcExp_120 = atoi(tttmp);	//120¼¶ÒÔºóµÄ¾­Ñé±¶Êý
	GetPrivateProfileString("¾­Ñé±¶Êý","iNpcExp_125","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iNpcExp_125 = atoi(tttmp);	//125¼¶ÒÔºóµÄ¾­Ñé±¶Êý
	GetPrivateProfileString("¾­Ñé±¶Êý","iNpcExp_130","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iNpcExp_130 = atoi(tttmp);	//130¼¶ÒÔºóµÄ¾­Ñé±¶Êý
	GetPrivateProfileString("¾­Ñé±¶Êý","iNpcExp_140","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iNpcExp_140 = atoi(tttmp);	//140¼¶ÒÔºóµÄ¾­Ñé±¶Êý
	GetPrivateProfileString("¾­Ñé±¶Êý","iNpcExp_150","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iNpcExp_150 = atoi(tttmp);	//150¼¶ÒÔºóµÄ¾­Ñé±¶Êý
	GetPrivateProfileString("¾­Ñé±¶Êý","iNpcExp_155","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iNpcExp_155 = atoi(tttmp);	//155¼¶ÒÔºóµÄ¾­Ñé±¶Êý
	GetPrivateProfileString("¾­Ñé±¶Êý","iNpcExp_160","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iNpcExp_160 = atoi(tttmp);	//160¼¶ÒÔºóµÄ¾­Ñé±¶Êý

	GetPrivateProfileString("¹ÖÎï","ï¯Ê¯À¬»øÊôÐÔ","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->JXSX_KG = atoi(tttmp);
	GetPrivateProfileString("¹ÖÎï","BOSSï¯Ê¯ÊôÐÔ","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->BOSSHS_KG = atoi(tttmp);
	GetPrivateProfileString("¹ÖÎï","¾­ÑéµÈ¼¶ÏÞÖÆ","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iJydjxz = atoi(tttmp);	
	GetPrivateProfileString("¹ÖÎï","µô±¦µÈ¼¶ÏÞÖÆ","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iGwdbxz = atoi(tttmp);	
	GetPrivateProfileString("¹ÖÎï","Õ½¶·¾­ÑéµÈ¼¶","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->iZdJyxz = atoi(tttmp);
	GetPrivateProfileString("¹ÖÎï","µô±¦ÊôÐÔÅÅÊý","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->m_zDLPS = atoi(tttmp);
	GetPrivateProfileString("¹ÖÎï","µô±¦ÊôÐÔÖØµþ","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->m_zSXCD = atoi(tttmp);
	GetPrivateProfileString("¹ÖÎï","ÐÒÔËµôÀ¶É«±¶Êý","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->XY_LSDB = atoi(tttmp);
	GetPrivateProfileString("¹ÖÎï","ÐÒÔËµô½ðÉ«±¶Êý","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->XY_HSDB = atoi(tttmp);
	GetPrivateProfileString("¹ÖÎï","·ÇÐÒÔËµôÀ¶É«±¶Êý","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->FXY_LSDB = atoi(tttmp);
	GetPrivateProfileString("¹ÖÎï","·ÇÐÒÔËµô½ðÉ«±¶Êý","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->FXY_HSDB = atoi(tttmp);
	GetPrivateProfileString("¹ÖÎï","±©³öÎïÆ·ÊýÁ¿","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->GW_BaoLv = atoi(tttmp);

	GetPrivateProfileString("¿¹×ªÄ§", "¿¹×ªÄ§¿ª¹Ø","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->KZM_KG = atoi(tttmp);
	GetPrivateProfileString("¿¹×ªÄ§", "¿¹×ªÄ§°Ù·Ö±È","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->KZM_BFB = atoi(tttmp);

	GetPrivateProfileString("ÎïÀí¹¥»÷ËÙ¶È", "¹¥»÷ËÙ¶È","",tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->AttackDelay = atoi(tttmp);

	GetPrivateProfileString("ÔÚÏßÅÝÔª±¦", "ÅÝÔª±¦¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->PYB_KG = atoi(tttmp);
	GetPrivateProfileString("ÔÚÏßÅÝÔª±¦", "ÅÝÔª±¦µÈ¼¶",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->PYB_DJ = atoi(tttmp);
	GetPrivateProfileString("ÔÚÏßÅÝÔª±¦", "ÅÝÔª±¦Ê±¼ä",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->BASIC_ADD_YB_TIME = atoi(tttmp);
	GetPrivateProfileString("ÔÚÏßÅÝÔª±¦", "ÅÝÔª±¦ÊýÁ¿",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->PYB_ShuLiang = atoi(tttmp);

	GetPrivateProfileString("ÔÚÏßÅÝ¾­Ñé", "ÅÝ¾­Ñé¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->EXP_KG = atoi(tttmp);

	GetPrivateProfileString("ÔÚÏßÅÝ¾öÕ½±Ò", "ÅÝ¾öÕ½±Ò¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->PJB_KG = atoi(tttmp);
	GetPrivateProfileString("ÔÚÏßÅÝ¾öÕ½±Ò", "ÅÝ¾öÕ½±ÒµÈ¼¶",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->PJB_DJ = atoi(tttmp);
	GetPrivateProfileString("ÔÚÏßÅÝ¾öÕ½±Ò", "ÅÝ¾öÕ½±ÒÊ±¼ä",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->BASIC_ADD_JB_TIME = atoi(tttmp);
	GetPrivateProfileString("ÔÚÏßÅÝ¾öÕ½±Ò", "ÅÝ¾öÕ½±ÒÊýÁ¿",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->PJB_ShuLiang = atoi(tttmp);

	GetPrivateProfileString("½»Ò×ÃÜÂë", "±£»¤¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->JYMM_KG = atoi(tttmp);

	GetPrivateProfileString("¿ª¹Ø", "Ë«±¶¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->SBEXP_KG = atoi(tttmp);
	GetPrivateProfileString("¿ª¹Ø", "Ñ£ÔÎ¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->GJXY_KG = atoi(tttmp);
	GetPrivateProfileString("¿ª¹Ø", "ÁÄÌì¹ýÂË¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->DLGL_KG = atoi(tttmp);
	GetPrivateProfileString("¿ª¹Ø", "»úÐµÈÎÎñ¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->JXRW_KG = atoi(tttmp);
	GetPrivateProfileString("¿ª¹Ø", "80¼¶Ä§·¨ÈÎÎñ¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->BSJMF_KG = atoi(tttmp);
	GetPrivateProfileString("¿ª¹Ø", "¶ªÆúÏÔÊ¾µØÃæ¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->DQXSDM_KG = atoi(tttmp);
	GetPrivateProfileString("¿ª¹Ø", "PKµôÂäÏÔÊ¾µØÃæ¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->PKDLXS_KG = atoi(tttmp);
	GetPrivateProfileString("¿ª¹Ø", "³ÆºÅÏÔÊ¾µÈ¼¶¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->CH_lV_KG = atoi(tttmp);

	GetPrivateProfileString("¶ñÄ§¹ã³¡", "ÃÅÆ±¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->EM_ZDKG = atoi(tttmp);
	GetPrivateProfileString("¶ñÄ§¹ã³¡", "¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->EM_KG = atoi(tttmp);

	GetPrivateProfileString("PK´óÈü", "×Ô¶¯¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->PK_ZDKG = atoi(tttmp);
	GetPrivateProfileString("PK´óÈü", "±ÈÈü¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->PK_DSKG = atoi(tttmp);
	GetPrivateProfileString("PK´óÈü", "É±ÈË½±Àø",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->PK_JIANG = atoi(tttmp);

	GetPrivateProfileString("»ØÊÕ", "ËÄÅÅ¼Û¸ñ",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->JG_SIPAIJ = atoi(tttmp);
	GetPrivateProfileString("»ØÊÕ", "°×ï¯¼Û¸ñ",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->JG_BAIGAO = atoi(tttmp);

	GetPrivateProfileString("¾üÍÅ", "ÒªÈûÏà¸ôÌìÊý",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->JT_YSTS = atoi(tttmp);
	GetPrivateProfileString("¾üÍÅ", "ÏÔÊ¾¾üÍÅÍ¼±ê",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->JT_Fortressid = atoi(tttmp);

	GetPrivateProfileString("ÕÙ»½", "¹ÖÎï¿ªÊ¼ID",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->ZH_KQhead = atoi(tttmp);//¿ªÆô±¦Ïä»ñµÃÎïÆ·¿ªÊ¼ID
	GetPrivateProfileString("ÕÙ»½", "¹ÖÎï½áÊøID",  "", tttmp.GetBuffer(MAX_PATH), 24,szfilename);     fcini->ZH_KQfoot = atoi(tttmp);//¿ªÆô±¦Ïä»ñµÃÎïÆ·½áÊøID
	f_Markini.Add(fcini);
	return TRUE;
}
BOOL CServerDlg::GetLoadMarkGMINI()//GMÃüÁîiniÎÄ¼þ
{
	TCHAR strPath[_MAX_PATH + 1];
	::GetCurrentDirectory(_MAX_PATH, strPath);
	CString szfilename = strPath;
	szfilename += _T("\\ÃüÁîÉèÖÃ.ini");	
	CString tttmp;
		
		MarkGMIni *fcini = new MarkGMIni;
		GetPrivateProfileString("GMÃüÁî", "Ë¢ÎïÆ·",  "", tttmp.GetBuffer(MAX_PATH),MAX_PATH, szfilename);     strcpy(fcini->Make,tttmp);//Ë¢×°±¸
		GetPrivateProfileString("GMÃüÁî", "Ë¢Ç®",  "", tttmp.GetBuffer(MAX_PATH),MAX_PATH, szfilename);     strcpy(fcini->AddGold,tttmp);//Ë¢Ç®
		GetPrivateProfileString("GMÃüÁî", "ºÏ³É",  "", tttmp.GetBuffer(MAX_PATH),MAX_PATH, szfilename);     strcpy(fcini->Compound,tttmp);//ºÏ³É
		GetPrivateProfileString("GMÃüÁî", "ÌßµôÍæ¼Ò",  "", tttmp.GetBuffer(MAX_PATH),MAX_PATH, szfilename);     strcpy(fcini->Kick,tttmp);//ÌßÈË
		GetPrivateProfileString("GMÃüÁî", "È«²¿ÏÂÏß",  "", tttmp.GetBuffer(MAX_PATH),MAX_PATH, szfilename);     strcpy(fcini->AllKick,tttmp);//È«ÏÂÏß
		GetPrivateProfileString("GMÃüÁî", "È«²¿ÒÆ¶¯",  "", tttmp.GetBuffer(MAX_PATH),MAX_PATH, szfilename);     strcpy(fcini->RecallAround,tttmp);//¸úÎÒ×ß
		GetPrivateProfileString("GMÃüÁî", "ÒþÉí",  "", tttmp.GetBuffer(MAX_PATH),MAX_PATH, szfilename);     strcpy(fcini->ObServer,tttmp);//ÒþÉí
		GetPrivateProfileString("GMÃüÁî", "ÍË³öÒþÉí",  "", tttmp.GetBuffer(MAX_PATH),MAX_PATH, szfilename);     strcpy(fcini->OUTObServer,tttmp);//ÍË³öÒþÉí
		GetPrivateProfileString("GMÃüÁî", "·Éµ½NPC",  "", tttmp.GetBuffer(MAX_PATH),MAX_PATH, szfilename);     strcpy(fcini->Tracenpc,tttmp);//·Éµ½NPC
		GetPrivateProfileString("GMÃüÁî", "·Éµ½Íæ¼Ò",  "", tttmp.GetBuffer(MAX_PATH),MAX_PATH, szfilename);     strcpy(fcini->Traceuser,tttmp);//·Éµ½Íæ¼Ò
		GetPrivateProfileString("GMÃüÁî", "ÒÆ¶¯",  "", tttmp.GetBuffer(MAX_PATH),MAX_PATH, szfilename);     strcpy(fcini->Link,tttmp);//ÒÆ¶¯
		GetPrivateProfileString("GMÃüÁî", "²é¿´ÈËÊý",  "", tttmp.GetBuffer(MAX_PATH),MAX_PATH, szfilename);     strcpy(fcini->HumanCount,tttmp);//²é¿´ÈËÊý
		GetPrivateProfileString("GMÃüÁî", "¹Ø½ø¼àÓü",  "", tttmp.GetBuffer(MAX_PATH),MAX_PATH, szfilename);     strcpy(fcini->Jail,tttmp);//¼àÓü
		GetPrivateProfileString("GMÃüÁî", "·Å³ö¼àÓü",  "", tttmp.GetBuffer(MAX_PATH),MAX_PATH, szfilename);     strcpy(fcini->UJail,tttmp);//·ÅÈË
		GetPrivateProfileString("GMÃüÁî", "ÅÝ¾­Ñé¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH),MAX_PATH, szfilename);     strcpy(fcini->PaoEXP,tttmp);//ÅÝ¾­Ñé¿ª¹Ø
		GetPrivateProfileString("GMÃüÁî", "ÅÝÔª±¦¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH),MAX_PATH, szfilename);     strcpy(fcini->PaoUYB,tttmp);//ÅÝÔª±¦¿ª¹Ø
		GetPrivateProfileString("GMÃüÁî", "Ë«±¶¿ª¹Ø",  "", tttmp.GetBuffer(MAX_PATH),MAX_PATH, szfilename);     strcpy(fcini->PaoSBK,tttmp);//Ë«±¶¿ª¹Ø

		GM_Markini.Add(fcini);
	return TRUE;
}
...
//	USER *pUser = NULL;
	...
	if(g_fpShuaWuPin.m_hFile != CFile::hFileNull) g_fpShuaWuPin.Close();//Ë¢ÎïÆ·¼ÇÂ¼
	if(g_fpMarkDeBug.m_hFile != CFile::hFileNull) g_fpMarkDeBug.Close();//MarkJayÂ©¶´ÐÅÏ¢
	if(g_fpMarkShiQuLog.m_hFile != CFile::hFileNull) g_fpMarkShiQuLog.Close();//MarkJayÂ©¶´ÐÅÏ¢
	...
	for(i = 0; i < GM_Markini.GetSize();i++)
	{
		if(GM_Markini[i]) 
		{
			delete GM_Markini[i];
...
{
	CString szTemp = _T("");
	szTemp.Format(_T("·þÎñÆ÷: %d"), nIndex);
...
BOOL CServerDlg::GetOneNpcItemNewTable(struct drop_info *dropItem)
{
	...
	for(i = 0;i < dropItem->n;i++)
	{
		...
		if(dropItem->novelity[i].code1 > 256 || dropItem->novelity[i].code2 > 5)
		{
...
BOOL CServerDlg::PreTranslateMessage(MSG* pMsg) 
{
	if(pMsg->message == WM_KEYDOWN)
	{
		if(pMsg->wParam == VK_RETURN) 
		{
			...
			CWnd* pInput = GetDlgItem(IDC_EDIT_ANNOUNCE);//ÏµÍ³Ëµ»°
...
void CServerDlg::OnAnnounce()
{
	...
	GetDlgItemText(IDC_EDIT_ANNOUNCE, strMsg);//ÏµÍ³Ëµ»°
	...
	SetDlgItemText(IDC_EDIT_ANNOUNCE, _T(""));//ÏµÍ³Ëµ»°
}
...
BOOL CServerDlg::GetMagicItemTable()
{
	...
	try
	{
		...
		while( !MagicItemSet.IsEOF() )
		{
			...
			magic_item->m_sScore		= MagicItemSet.m_sScore;
			magic_item->m_sWeapScore	= MagicItemSet.m_sWeapScore;
			magic_item->m_sAid			= 0;
	...
	try
	{
		...
		while( !AccessoriUpSet.IsEOF() )
		{
			...
			if(AccUp->m_tGroup == 1) g_Ripel.m_arRipelCrest.Add(AccUp->m_sSid);//Ô­°æ±¸·Ý
...
BOOL CServerDlg::GetOnlineShopTable()
{
	...
	::ZeroMemory(szSQL, sizeof(szSQL));
	wsprintf(szSQL,TEXT("SELECT * FROM OnlineShop ORDER BY oSid"));
	...
	upgrade = 0;
	sx6 = 0;	sx7 = 0;	sx8 = 0;	sx9 = 0;	sx10 = 0; type = 0;
	...
	if( retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO )
	{
		while (TRUE)
		{
			...
			if (retcode ==SQL_SUCCESS || retcode ==SQL_SUCCESS_WITH_INFO)
			{
				...
				SQLGetData( hstmt, i++, SQL_C_ULONG, &price, sizeof(price), &sInd );
				...
				SQLGetData( hstmt, i++, SQL_C_SSHORT, &type, sizeof(type), &sInd );
...
/////////////////////////////////////////////////////////////////////////////////////
//	ÔÚÏß³é½±
BOOL CServerDlg::GetMarkChouJiangTable()
{
	int i;

	for(i = 0; i < g_arMarkChouJiangTable.GetSize(); i++) g_arMarkChouJiangTable.RemoveAll();
	...
	::ZeroMemory(szSQL, sizeof(szSQL));
	wsprintf(szSQL,TEXT("SELECT * FROM Mark_ChouJiang"));
	...
	{
		TRACE("Fail To Load UserSort Data !!\n");
		AfxMessageBox(_T("MarkChouJiang Table Open Fail!"));
		return FALSE;
	}
	...
	if( retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO )
	{
		while (TRUE)
		{
			...
			if (retcode ==SQL_SUCCESS || retcode ==SQL_SUCCESS_WITH_INFO)
			{
				...
				MarkChouJiang *OSP = new MarkChouJiang;
				...
				OSP->m_sx9	 = sx9;
				OSP->m_sx10	 = sx10;
				g_arMarkChouJiangTable.Add(OSP);
			}
			else break;
		}		
	}
...
/////////////////////////////////////////////////////////////////////////////////////
// »áÔ±³äÖµ
BOOL CServerDlg::GetVIPPayTable()
{
	int i;
...
/////////////////////////////////////////////////////////////////////////////////////
// ÔÚÏß³äÖµ
BOOL CServerDlg::GetMarkPayTable()
{
	int i;
...
/////////////////////////////////////////////////////////////////////////////////////
//	ÔÚÏß¹«¸æ
BOOL CServerDlg::GetMarkGonggaoTable()
{
	int i;
	for(i = 0; i < g_arMarkGonggaoTable.GetSize(); i++) g_arMarkGonggaoTable.RemoveAll();
	SQLHSTMT		hstmt;
	SQLRETURN		retcode;
	TCHAR			szSQL[2048];
	::ZeroMemory(szSQL, sizeof(szSQL));
	wsprintf(szSQL,TEXT("SELECT * FROM Mark_GongGao"));
	SQLCHAR			strGGSay[100];
	SQLINTEGER		sInd;
	::ZeroMemory(strGGSay, sizeof(strGGSay));
	int db_index = 0;
	CDatabase* pDB = g_DB[AUTOMATA_THREAD].GetDB( db_index );
	if( !pDB ) return FALSE;
	retcode = SQLAllocHandle( (SQLSMALLINT)SQL_HANDLE_STMT, pDB->m_hdbc, &hstmt );
	...
	{
		TRACE("Fail To Load UserSort Data !!\n");
		AfxMessageBox(_T("MarkGongGao Table Open Fail!"));
		return FALSE;
	}
	retcode = SQLExecDirect( hstmt, (unsigned char*)szSQL, SQL_NTS);
	if( retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO )
	{
		while (TRUE)
		{
			...
			{
				i = 1;
				SQLGetData( hstmt, i++, SQL_C_CHAR,	  &strGGSay, sizeof(strGGSay), &sInd );
				MarkGonggao *OSP = new MarkGonggao;
				strcpy(OSP->m_strGGSay, (CHAR*)strGGSay);
				g_arMarkGonggaoTable.Add(OSP);
			}
			else break;
		}		
	}
...
void CServerDlg::OnRefresh() 
{
	...
	char tsbuf[100];
	if ( AUTOMATA_THREAD == 4 )//²âÊÔÐÞ¸´µô¶Ë
	...
	wsprintf(strTitle, _ID( IDS_SERVERDLG_CURRENTUSER ), nCount);//"Mark×¨ÒµÒýÇæ-µ±Ç°ÈËÊýÎª: %d"
...
//	OnRefresh();
	...
	case	10002:
		...
		{
			m_editCtrl = "Î´Á¬½Óµ½BridgeServer...(TIMER)\r\n";	UpdateData( FALSE );
			...
			CString str = _T("");
			str.Format("%s live = %d Event = %d Òì³£\r\n",strDate,GetTickCount() - m_dwBSAlive,nIDEvent);
		...
		{
			m_editCtrl.Format( "ÒÑÁ¬½Óµ½BridgeServer...(TIME:%d)\r\n", GetTickCount()-m_dwBSAlive );//	UpdateData( FALSE );
		}
		break;
	default:
		break;
...
//		pUser->SockCloseProcess();
	...
	Sleep(2000);//´¦ÀíËùÓÐÏß³ÌÍË³ö 
}

void CServerDlg::OnCancel()
{
	USER* pUser = NULL; //·þÎñÆ÷¹Ø±ÕÊ±ÌßËùÓÐÔÚÏßÈËÎï
	for(int i = 0; i < MAX_USER; i++)
	{
		pUser = m_Com.GetUserUid(i);
...
//		DisplayErrorMsg(hstmt);
	...
	//if(sRet = 0) return FALSE;//MarkJay ÐÞ¸Ä
...
//	m_pBridgeSocket = NULL;
	...
	if ( !pSocket->Create() )
	{
		...
		m_editCtrl = "Î´Á¬½Óµ½BridgeServer...\r\n";	UpdateData( FALSE );
		return FALSE; 
	}
	...
	m_editCtrl = "ÒÑÁ¬½Óµ½BridgeServer\r\n";	UpdateData( FALSE );
...
void CServerDlg::BridgeServerDoubleAccountClose(TCHAR *pBuf)
{
	...
	if( uid == -1 )
	{
		...
		for( int i = 0; i < MAX_USER; i++)
		{
			...
			if( !strSource.CompareNoCase(pUser->m_strAccount))
			{
				...
				if( pUser->m_state != STATE_DISCONNECTED && pUser->m_state != STATE_LOGOUT)
				{
					pUser->SendSystemMsg( IDS_USER_DOUBLE_ACCOUNT, SYSTEM_SPECIAL, TO_ME);//"´ËÕÊ»§ÕýÔÚÁ¬½ÓÖÐ."
					pUser->SoftClose();
					break;
				}
			}
	...
	pUser->SendSystemMsg( IDS_USER_DOUBLE_CHAR, SYSTEM_SPECIAL, TO_ME);//"´ËÈËÎïÒÑ±»Ê¹ÓÃ."
	pUser->SoftClose();
}
...
void CServerDlg::BridgeDeputeGuildMasterOfPowerResult(TCHAR *pBuf)
{
	...
	else												// ´Ù¸£´Ù. ´Ù¸¥ ¼­¹ö°¡ ¿äÃ»ÇÑ °á°ú¸¦ ¹Þ¾Æ ¸Þ¸ð¸®¸¸ ¼ÂÆÃÇÑ´Ù.
	{
		...
		CString strMsg = _T("");
		strMsg.Format( IDS_USER_GUILD_SUBMASTER_ON, strSubMasterID);//"%s ±»ÈÎÃüµ£ÈÎ¾üÍÅ¸±ÍÅ³¤."
...
void CServerDlg::BridgeRemoveGuildMasterOfPowerResult(TCHAR *pBuf)
{
	...
	else												// ´Ù¸£´Ù. ´Ù¸¥ ¼­¹ö°¡ ¿äÃ»ÇÑ °á°ú¸¦ ¹Þ¾Æ ¸Þ¸ð¸®¸¸ ¼ÂÆÃÇÑ´Ù.
	{
		...
		CString strMsg = _T("");
		strMsg.Format( IDS_USER_GUILD_SUBMASTER_OFF, strSubMasterID);// "%s ±»È¡Ïûµ£ÈÎ¾üÍÅ¸±ÍÅ³¤."
...
void CServerDlg::BridgeServerGuildOffResult(TCHAR *pBuf)
{
	...
	else												// ´Ù¸£´Ù. ´Ù¸¥ ¼­¹ö°¡ ¿äÃ»ÇÑ °á°ú¸¦ ¹Þ¾Æ ¸Þ¸ð¸®¸¸ ¼ÂÆÃÇÑ´Ù.
	{
		...
		CString strMsg = _T("");
		strMsg.Format( IDS_USER_OUT_MEMBER, strOffID);//"%s ÒÑÍË³ö¾üÍÅ."
	...
	else												// ´Ù¸£´Ù. ´Ù¸¥ ¼­¹ö°¡ ¿äÃ»ÇÑ °á°ú¸¦ ¹Þ¾Æ ¸Þ¸ð¸®¸¸ ¼ÂÆÃÇÑ´Ù.
	{
		...
		for(int i = 0; i < g_arGuildHouse.GetSize(); i++)
		{
			if(pGuild->m_lSid == g_arGuildHouse[i]->iGuild)
			{
				...
				pNpc->FillNpcInfo(&m_Com, modify_send, modify_index, INFO_MODIFY);
...
//////////////////////////////////////////////////////////////////////////
// ¼ì²âIP°×Ãûµ¥ MarkJay
BOOL CServerDlg::CheckUserIP(TCHAR *UserIP)
{
	SQLHSTMT		hstmt;
	SQLRETURN		retcode;
	TCHAR			szSQL[1024];
	...
	memset( szSQL, 0x00, 1024 );
	_sntprintf(szSQL, sizeof(szSQL), TEXT("{call IS_EXIST_CHAR_IP (\'%s\', ?)}"),UserIP);
...
void CServerDlg::OnBnClickedButtonDrop() //ÖØÔØµô±¦ ÐÞ²¹
{
    int number, i;
	char szfilename[MAX_PATH + 1];
	GetCurrentDirectory(MAX_PATH,szfilename);//»ñÈ¡³ÌÐòµÄµ±Ç°Ä¿Â¼
	strcat(szfilename,"\\drop.ini");
	char tmp[512] = {0};
	byte code1,code2;
	FILE *fp = NULL;
	char n1[20] = {0}, n2[20] = {0}, n3[20] = {0}, n4[20] = {0}, n5[20] = {0}, n6[20] = {0};
