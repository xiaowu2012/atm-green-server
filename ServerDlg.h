...
public:
	...
	void AddSkillTable();
	BOOL GetOnlineShopTable();//在线商店
	BOOL GetVIPPayTable();//会员充值
	BOOL GetMarkPayTable();//在线充值
	BOOL GetMarkGonggaoTable();//在线公告
	BOOL GetMarkChouJiangTable();//在线抽奖
...
public:
	...
	BOOL GetLoadMarkJzINI();
	BOOL GetLoadMarkGMINI();
	BOOL CheckUserIP(TCHAR* UserIP);//检测IP白名单
...
public:
	afx_msg void OnBnClickedQuanout();
    afx_msg void OnBnClickedButtonDrop();//重载掉宝  小黑
};
